...
function initializeOptionBuffer(classicOptions,optionType,optionBuffer){	  
	  for (var i = 0; i < classicOptions.length; i++)
	  {
		classicOption = classicOptions[i];  
		var type = "";
		if( optionType == "option_data__" ||optionType == "option_collection__" ||optionType == "option_application__" ){
			 type = "data";
		}else if( optionType == "option_service__" ){
			type = "service";
		}else if( optionType == "option_any" ){
			type = "common";
		}
			if (classicOption.getAttribute("queryableType") != null && (classicOption.getAttribute("queryableType") == "common" || classicOption.getAttribute("queryableType") == type))
	        {
				if (classicOption.id != "")
				{
				  var newId = optionType + classicOption.id;
...
function copyStandartsOptions ()
{
  ...
  if(optionBuffer != null){
	initializeOptionBuffer(classicOptions,"option_any__",optionBuffer);
  }
  */
  for (var i = 0; i < arrayBuffer.length; i++){			
		var optionsBuff = document.getElementById(arrayBuffer[i]);    
		if (optionsBuff != null) {	  			
			initializeOptionBuffer(classicOptions,"option_"+arrayTabType[i]+"__",optionsBuff);
		} 
  }   
}
...
function removeAllOptions() {
	...
	var optionBufferForDataset = document.getElementById("optionsBufferDataset");
   
    if (optionBufferForDataset == null) {return (false);}   
    var optionsDataset = optionBufferForDataset.childNodes;    
    for (var i = 0; i < optionsDataset.length; i++)
    {	
     	var optionData = optionsDataset[i];    
     	if (optionData.id != null) {			
     	   removeOptionSelect(optionData.id);
     	}
    }
	
	var optionBufferForCollection = document.getElementById("optionsBufferCollection");
   
    if (optionBufferForCollection == null) {return (false);}   
    var optionsDataset = optionBufferForCollection.childNodes;    
    for (var i = 0; i < optionsDataset.length; i++)
    {	
     	var optionData = optionsDataset[i];    
     	if (optionData.id != null) {			
...
function addElements2ComboBox(optionBuffer,type,index){
	var options = optionBuffer.childNodes;
	
	
	var queryableType = "";  
	  if ((type == "data") || (type == "application") || (type == "collection") ) {
		 queryableType = "data";
	  } else if (type == "service") {
		queryableType = "service";
	  } else if (type ="any") {
		queryableType = "common";
	  }
	...
	for (var i = 0; i < options.length; i++){	
    	...
    	if (option.id != null) {			
	        ...
	        if (optionP.getAttribute("queryableType") != null && (optionP.getAttribute("queryableType") == "common" || optionP.getAttribute("queryableType") == queryableType))
	        {
					addOption4Select(newId,type,index);				
	        }
	        ...
	        //check for mandatory elements
      		...
      		{	
					addOption4AllTab(option.id,index);						
					i--;
      		}else if (optionP.getAttribute("exception") != null && optionP.getAttribute("exception") == type)
      		{	
					addOption4AllTab(option.id,index);	
					i--;					
      		}
...
function initOptionSelect1(selectedType) {
	
}

function prepareForInitial(arrBuffer,arrOption,arrTabId,arrTabType){
	arrayBuffer = arrBuffer;
	arrayOption = arrOption;
	arrayTabId = arrTabId;	
	arrayTabType = arrTabType;	
	
	 
	
}
function initOptionSelect(selectedType)
{  
	/*var queryableType = "";  
	if ((selectedType == "dataset") || (selectedType == "application") || (selectedType == "collection")) {
		 queryableType = "data";
	...
	copyStandartsOptions ();
	 ...
	 for (var i = 0; i < arrayBuffer.length; i++){			
		...
		if (optionsBuff != null) {	  
			...
			addElements2ComboBox(optionsBuff,arrayTabType[i],i);
		} 
	 } 
	 /*var optionBuffer = document.getElementById("optionsBuffer");    
	 if (optionBuffer!= null) {		
		addElements2ComboBox(optionBuffer,"any");
	 }  */
}
...
function addOptionSelect (optionId)
{
  ...
  for ( i = 0; i < optionContents.length; i++ )
  {
    ...
    if(optionContent.tagName.toUpperCase() == "P")
    {
      var optionText = "";
      for (var j=0; j<optionContent.childNodes.length; j++)
      {
        var nodeContent = optionContent.childNodes[j];
        if(nodeContent.nodeType == 1 && nodeContent.tagName.toUpperCase() == "LABEL") 
        {
...
function  addOption4Select (optionId,type,index){
	...
	optionSelectOpt = document.getElementById(arrayOption[index]);   	
	option = document.getElementById("option_"+arrayTabType[index]+"__" + optionId);
	...
	if ( optionSelectOpt == null || option == null ){	
		...
		element.value = "option_"+arrayTabType[index]+"__" + optionId;		
...
function removeOption4AllTab (optionId,type)
{ 
	 var option = document.getElementById(optionId);
	 var optionsBuffer; 
	 
	 optionsBuffer = document.getElementById(arrayBuffer[type]);
	 ...
	 var optionContents = option.childNodes;
	  ...
	  for (var i = 0; i < optionContents.length; i++)
	  {
		...
		if ( optionContent.tagName.toUpperCase() == "SPAN" && optionContent.name == "removeButton")
		{	  
	  ...
	  li.id = "option_"+arrayTabType[type]+"__" + optionId;
	 	  
	  li.appendChild(option);    
	  optionsBuffer.appendChild(li);  
	  addOption4Select(optionId,arrayTabType[type],type);	
  
}

function addOption4AllTab (mandatoryId,type) {
  ...
  {
	   var selected;		
	   selected = document.getElementById (arrayOption[type]);    
  ...
  var panel = document.getElementById(arrayTabId[type]);
  ...
  optionToAdd.id = arrayTabType[type] +"__" + selectedOption.split("__")[1];
  
  var prePart =  arrayTabType[type];
  var nexPart =  selectedOption.split("__")[1];
  
  var buffer = document.getElementById(arrayBuffer[type]);
  var items = buffer.childNodes;
  
  
  
   if(nexPart == "denominatorP"){
		var distanceUOM ="option_" + arrayTabType[type] + "__distanceUOMP";
		var distanceVal ="option_" + arrayTabType[type] + "__distanceValP";
		var counter =0;
		for (var i = 0; i < items.length; i++)
		{   
			if( (items[i].id == distanceVal) || (items[i].id == distanceUOM)  ){
			   counter = counter + 1;
			}
			
		}
		if(counter < 2 ){
				alert ('Only used, if DistanceValue and DistanceUOM are not used.');
				return (false);
		}
		
		//var distanceUOM = document.getElementById(arrayTabType[type] +'__distanceUOMP');
		//var distanceVal = document.getElementById(arrayTabType[type] +'__distanceValP');
		
		//var items = buffer.childNodes;
		
		//if( distanceUOM != null || distanceVal  != null ){			
		//	alert ('Only used, if DistanceValue and DistanceUOM are not used.');
		//	return false;
		//}
   
   }else if (nexPart =='distanceUOMP' || nexPart =='distanceValP' ){		
		var denominator = "option_" + arrayTabType[type]  +'__denominatorP';		
				
		var counter =0;
		for (var i = 0; i < items.length; i++)
		{   
			if( (items[i].id == denominator) ){
			   counter = counter + 1;
			}			
		}
		if(counter == 0  ){
				alert ('Only used, if Denominator is not used');
				return (false);
		}		
   }
...
function initValidation ()
{
  ...
  var title_advanced = document.getElementById("title_advanced");
  var title_data = document.getElementById("title_data");
  var title_collection = document.getElementById("title_collection");
  var title_application = document.getElementById("title_application");
  var title_service = document.getElementById("title_service");
    
  var target;

  if (title_classic != null && title_classic.className == "selected")
  {	
    target = document.getElementById("classic");
  }else if (title_advanced != null && title_advanced.className == "selected")
  {	
    target = document.getElementById("advanced");
  }else if (title_data != null && title_data.className == "selected")
  {	
    target = document.getElementById("data");
  }else if (title_collection != null && title_collection.className == "selected")
  {	
    target = document.getElementById("collection");
  }else if (title_application != null && title_application.className == "selected")
  {	
    target = document.getElementById("application");
  }else if (title_service != null && title_service.className == "selected")
  {	
    target = document.getElementById("service");
  }
  ...
  for (var i = 0; i < elements.length; i++)
  {
    ...
    if (element.tagName.toUpperCase() == "P" && element.id != null)
    {
      ...
      for (var j = 0; j < childs.length; j++)
      {
        var child = childs[j];
        if (
            (child.tagName.toUpperCase() == "INPUT" || child.tagName.toUpperCase() == "SELECT") 
...
function checkOptionsFields()
{
 ...
 // var massForm = mass[0];
  ...
  for (j=0; j<fieldsets.length ; j++)
  {
    ...
    if (fieldset.tagName.toUpperCase() == "INPUT")
    {
      addUsedParametersField( item + "__" +fieldset.id, fieldset.value);
    }

    if (fieldset.tagName.toUpperCase() == "SELECT")
    {
    ...
    if (fieldset.tagName.toUpperCase() == "FIELDSET")
    {
      ...
      if (fieldset.id == toGet)
      {
        ...
        for (k = 0; k < paragraphs.length; k++ )
        {
            ...
            if (paragraph.tagName.toUpperCase() == "P")
            {
