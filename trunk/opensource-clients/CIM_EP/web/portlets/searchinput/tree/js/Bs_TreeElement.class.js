...
//...rse
  ...
  this.render =
    function(omitDivTags, putIntoPage, lookAhead) {
      if (typeof(this._tree.stopWatch) == 'object') 
        this._tree.stopWatch.takeTime('Bs_TreeElement.render() for id: '
                                      + this.id + ' in level: ' + this._level);
      ...
      if ((this._level) > 0 || (this._tree.showPseudoElement)) {
        ...
        out[outI++] = '>';
        out[outI++] = '<div style="overflow:visible; ' + this._getVar('divStyle') + '">';
        ...
        if (this.hasVisibleChildren()) {
          if ((this._level == 0) 
              || (!this._tree.showPseudoElement 
                 && (this._level == 1)
                 && ((this._tree.useAutoSequence && (this.id == 1)) || (!this._tree.useAutoSequence && true)))) {
            if (this.hasSiblingsDown()) {
              imgNumber++;
            }
            else {
...
//replace the string __this.id__ with the actual id (int).
        ...
        out[outI++] = '<img id="' + 
                      ...
                      onClickStr + 
                      ' align="middle"';
        out[outI++] = ' style="vertical-align:' +
                      ((imageHeight > 16) ? 'middle' : 'top') +
                      '">';
...
//rse...
            t.checkboxValue = this.checkboxValue;
//...rse
            ...
            if (this._tree.checkboxSystemWalkTree) {
              t.attachOnClick('Bs_Objects['+this._tree._id+'].elementCheckboxEvent(\'' +
                                                                                    this.id + 
                                                                                    '\', ' + 
                                                                                    checkboxObj + 
                                                                                    '.value);');
          ...
          if (hRef)
            out[outI++] = hRef;
          out[outI++] = '<span id="' + this._tree._objectId + '_e_' + this.id + '_caption2">' + this.caption + '</span>';
          if (hRef)
            out[outI++] = '</a>';
          out[outI++] = '</span>';
          if (this.onClick) {
...
//rse...
      if (typeof(a['checkboxValue']) != 'undefined') {
        this.checkboxValue  = a['checkboxValue'];
      }
//...rse
...
//rse...
      if (typeof(this.checkboxValue)!= 'undefined') ret['checkboxValue'] = this.checkboxValue;
//...rse
...
//rse...
        if (this.checkboxValue)     ret += varName + "['checkboxValue']      = '" + this.checkboxValue      + "';\n";
//...rse
  ...
  this._getSourceOpenCloseIcon =
    function() {
      ...
      if (this.hasVisibleChildren()) {
        if ((this._level == 0) ||
            (!this._tree.showPseudoElement && (this._level == 1) && ((this._tree.useAutoSequence && (this.id == 1)) || (!this._tree.useAutoSequence && true)))) {
          if (this.hasSiblingsDown()) {
            imgNumber++;
          }
          else {
...
//ev = ev.replace(/__this__/g, 'this');
//replace the string __this__ with 'this'.
            eval(ev);
          }
        }
      }
  ...
  this.onMouseOver =
    function() {
      ...
      if (!img.swapOver0) {
        ...
        img.swapOver0.src = this.imgDir + 'enabled_0_over.gif';img.swapOver1 = new Image();
  ...
  this.setCheckboxValue =
    function(value, fireEvents, doWalk) {
  ...
  this._updateCheckboxFromParent =
    function(newValue, recursiveDown) {
